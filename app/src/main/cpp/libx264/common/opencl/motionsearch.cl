...
int sad_8x8_ii_coop4(read_only image2d_t fenc, int2 fencpos, read_only image2d_t fref, int2 frefpos,
                     int idx, local int16_t
...
.x == mvp_buffer[mb_xy].
x &&mvp
.y == mvp_buffer[mb_xy].y )
{
out_mvs[mb_xy] = in_mvs[mb_xy];
return;
}
mvp_buffer[mb_xy] =
convert_short2_sat(mvp);
...
int2 prevmv = b_shift_index ? convert_int2_sat(
        in_mvs[find_downscale_mb_xy(mb_x, mb_y, mb_width, mb_height)]) : convert_int2_sat(
        in_mvs[mb_xy]);
prevmv >>=
scale;
trymv = prevmv;
