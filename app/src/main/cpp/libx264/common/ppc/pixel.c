...
static void pixel_sad_x4_16x16_altivec(uint8_t *fenc,
    ...
    for (int y = 0; y < 8; y++) {
        ...
        sum0v = (vec_s32_t) vec_sum4s(vec_sub(vec_max(fencv, pix0v), vec_min(fencv, pix0v)),
                                      (vec_u32_t) sum0v);
        sum1v = (vec_s32_t) vec_sum4s(vec_sub(vec_max(fencv, pix1v), vec_min(fencv, pix1v)),
                                      (vec_u32_t) sum1v);
        sum2v = (vec_s32_t) vec_sum4s(vec_sub(vec_max(fencv, pix2v), vec_min(fencv, pix2v)),
                                      (vec_u32_t) sum2v);
        sum3v = (vec_s32_t) vec_sum4s(vec_sub(vec_max(fencv, pix3v), vec_min(fencv, pix3v)),
                                      (vec_u32_t) sum3v);
...
static uint64_t x264_pixel_var_8x8_altivec(uint8_t *pix, intptr_t i_stride) {
    ...
    static const vec_u8_t perm_tab[] =
            {
                    CV(0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06,
                       0x07,  /* pix=mod16, i_stride=mod16 */
                       0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17),
                    CV(0x08, 0x09, 0x0A, 0x0B, 0x0C, 0x0D, 0x0E,
                       0x0F,  /* pix=mod8, i_stride=mod16  */
...
static int pixel_sa8d_16x16_altivec(uint8_t *pix1, intptr_t i_pix1,
    ...
    i_satd = (pixel_sa8d_8x8_core_altivec(&pix1[0], i_pix1, &pix2[0], i_pix2)
              ...
              + pixel_sa8d_8x8_core_altivec(&pix1[8 * i_pix1 + 8], i_pix1, &pix2[8 * i_pix2 + 8],
                                            i_pix2) + 2) >> 2;
    return i_satd;
}
