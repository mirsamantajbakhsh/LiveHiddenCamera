...
void
x264_store_interleave_chroma_neon(pixel *dst, intptr_t i_dst, pixel *srcu, pixel *srcv, int height);
...
void
x264_frame_init_lowres_core_neon(uint8_t *, uint8_t *, uint8_t *, uint8_t *, uint8_t *, intptr_t,
                                 intptr_t, int, int);

void
x264_mbtree_propagate_cost_neon(int16_t *, uint16_t *, uint16_t *, uint16_t *, uint16_t *, float *,
                                int);
...
static void
(*const x264_pixel_avg_wtab_neon[6])(uint8_t *, intptr_t, uint8_t *, intptr_t, uint8_t *, int) =
        {
                NULL,
...
static uint8_t *get_ref_neon(uint8_t *dst, intptr_t *i_dst_stride,
    ...
    if (qpel_idx & 5) /* qpel interpolation needed */
    {
        ...
        x264_pixel_avg_wtab_neon[i_width >> 2](
                ...
                src2, i_height);
        if (weight->weightfn)
            weight->weightfn[i_width >> 2](dst, *i_dst_stride, dst, *i_dst_stride, weight,
                                           i_height);
        return dst;
    } else if (weight->weightfn) {
        weight->weightfn[i_width >> 2](dst, *i_dst_stride, src1, i_src_stride, weight, i_height);
